...
if ( ! class_exists( 'LaStudio_Pagespeed_Update' ) ) :
	...
	class LaStudio_Pagespeed_Update {
		 ...
		 * @type string
		 */
		const AJAX_ACTION = 'lastudio_pagespeed_plugin_update_dismiss_notice';
		 ...
		 * @var string
		 */
		private static $api_url = 'https://la-studioweb.com/file-resouces/shared/plugins/lastudio-pagespeed/update-check.json';

		private static $_plugin_slug = 'lastudio-pagespeed';

		private static $_plugin_url = 'https://la-studioweb.com';

		private static $_plugin_file = 'lastudio-pagespeed/lastudio-pagespeed.php';

		private static $_plugin_file_php = 'lastudio-pagespeed.php';

		private static $_plugin_name = 'LaStudio PageSpeed';

		private static $_plugin_opt_key = 'lastudio_pagespeed_plugin_state';

		/**
		 * The LaStudio_Pagespeed_Update Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see LaStudio_Pagespeed_Update()
		 * @uses LaStudio_Pagespeed_Update::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true LaStudio_Pagespeed_Update.
		 * @codeCoverageIgnore
		 */
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'lastudio-pagespeed' ), '1.0.0' );
		}
		...
		public function update_plugins( $transient ) {
			...
			if ( 'activated' === $state ) {
				$api_check = $this->api_check();
				if ( is_object( $api_check ) && version_compare( LASTUDIO_PAGESPEED_VERSION, $api_check->version, '<' ) ) {
		...
		public function notice() {
			...
			if ( 'deactivated' === $state ) {
				...
				$message = sprintf(
					esc_html__( '%1$sActivate the %1$3 plugin%2$s', 'lastudio-pagespeed' ),
				...
				$message = sprintf(
					esc_html__( '%1$sInstall the %3$s plugin%2$s', 'lastudio-pagespeed' ),
	...
	if ( ! function_exists( 'lastudio_pagespeed_plugin_update_instance' ) ) :
		/**
		 * LaStudio_Pagespeed_Update Instance
		 *
		 * @since 1.0.0
		 *
		 * @return LaStudio_Pagespeed_Update
		 */
		function lastudio_pagespeed_plugin_update_instance() {
			return LaStudio_Pagespeed_Update::instance();
		}
	endif;

	/**
	 * Loads the main instance of LaStudio_Pagespeed_Update
	 *
	 * @since 1.0.0
	 */
	add_action( 'after_setup_theme', 'lastudio_pagespeed_plugin_update_instance', 99 );
