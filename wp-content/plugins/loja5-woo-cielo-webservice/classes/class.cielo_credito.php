<?php   
	class WC_Gateway_Loja5_Woo_Cielo_Webservice extends WC_Payment_Gateway {
        ...
        public function __construct() {
			...
			$this->method_description = __( 'Ativa o pagamento por Cartão de Crédito via Cielo.', 'loja5-woo-cielo-webservice' );
            $this->method_title = 'Cielo API - Cr&eacute;dito';
            $this->init_settings();
        ...
        public function init_form_fields() {
			...
			if(file_exists(CIELO_WEBSERVICE_WOO_PATH.'/include/licenciamento.php')){
				...
				$this->form_fields = gateway_woo_cielo_webservice_loja5_licensa_credito($this);
			}else{
            ...
            if(isset($_POST['payment_method']) && $_POST['payment_method']=='loja5_woo_cielo_webservice'){
				...
				if($this->get_post('bandeira')=='visa' || $this->get_post('bandeira')=='mastercard' || $this->get_post('bandeira')=='elo'){
					if($this->settings['3ds']=='yes' && CIELO_WEBSERVICE_WOO_CREDITO_3DS && $this->settings['forcar_autenticar']=='no' && $this->get_post('auth')!='onSuccess'){
						$this->tratar_erro("Ops, n&atilde;o foi poss&iacute;vel autenticar seu cart&atilde;o junto ao emissor (".trim($this->get_post('auth'))."), isso significa que o mesmo n&atilde;o &eacute; autentic&aacute;vel ou a operadora n&atilde;o esta conseguindo realizar o processo de autentica&ccedil;&atilde;o do mesmo, tente outro ou escolha outra forma de pagamento!");
					}
				}
				//valida
                $erros = 0;
                ...
                if($this->settings['fraude']=='yes' && $currency_code=='BRL' && isset($_POST['country']) && $_POST['country']=='BR'){
        ...
        public function process_payment($order_id) {
			...
			//tentativas 
			$qtd_tentativas = (int)get_post_meta($order_id, '_tentativar_cielo_api', true);
			if($qtd_tentativas > 10){
				$erro = 'Número de tentativas ('.$qtd_tentativas.') de pagamento excedidas para este pedido, escolha outra forma de pagamento!';
				update_post_meta($order_id, 'erro_cielo', $erro);
				$this->tratar_erro($erro);
                return false;
			}
			...
			if($response->status==401){
				...
				if(isset($dados_pedido['Code'])){
					...
					//tentativas
					update_post_meta($order->get_id(),'_tentativar_cielo_api',($qtd_tentativas+1));
					//bloqueia a finalização de pedidos negados se ativado 
					if($dados_pedido['Payment']['Status']==3 && $this->settings['acao']=='alerta'){
						$erro = "Seu pedido foi negado por o emissor de seu cart&atilde;o, verifique se informou todos os dados corretamente e se preferir tente usando um novo cart&atilde;o.";
						$this->tratar_erro($erro);
						update_post_meta($order_id, 'erro_cielo', $erro);
						return false;
					}
					...
					update_post_meta($order_id,'_installment_number',$parcela);
					update_post_meta($order_id,'_processado_cielo_loja5','true');
					
					//clearsale
					update_post_meta( $order->get_id(), '_card_number', $bin );
					update_post_meta( $order->get_id(), '_card_bin', $bin );
					update_post_meta( $order->get_id(), '_card_year', $ano_cartao );
					update_post_meta( $order->get_id(), '_card_month', $mes_cartao );
					update_post_meta( $order->get_id(), '_card_name', $nome_completo );
