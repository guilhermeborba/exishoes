...
function qual_cartao_credito_cielo(cardNumber) {
	...
	if(cardNumber.length > 6){
		for (var t = numero, e = [
			...
			["hipercard", ["38", "60"]],
			["visa", ["4"]],
			["mastercard", ["5", "22", "23", "24", "25", "26", "27"]]
		], i = 0; i < e.length; i++)
...
function detectar_bandeira_cartao_credito_cielo_loja5(numero){
	...
	if(result!=''){
		...
		jQuery('#parcela-cielo-webservice-credito').html('<option value="">Clique na bandeira correspondente!</option>');
	}
}
...
function retorno_parcelamento(data) {
	...
	jQuery('#total-cielo-webservice').val(data.total);
}
</script>
...
<input type="hidden" name="cielo_webservice[hash_total_cielo]" value="<?php echo sha1(number_format($total_cart, 2, '.', ''));?>">
<input type="hidden" name="cielo_webservice[total_cielo]" value="<?php echo number_format($total_cart, 2, '.', '');?>">

<input type="hidden" name="cielo_webservice[total]" id="total-cielo-webservice" value="<?php echo number_format($total_cart,2,'.','');?>">
...
<p class="form-row form-row-wide woocommerce-validated campos_cielo_webservice">
<label style="padding: 5px 0 5px 5px;">CPF/CNPJ do titular:</label>
...
<select style="box-shadow: inset 2px 0 0 #0f834d;height:40px;width:100%;background-color: #f2f2f2;border: none;" name="cielo_webservice[validade_mes]" id="validade-cielo-webservice-mes">
...
<select style="box-shadow: inset 2px 0 0 #0f834d;height:40px;width:100%;background-color: #f2f2f2;border: none;" name="cielo_webservice[validade_ano]" id="validade-cielo-webservice-ano">
...
<select onchange="autenticar_credito(this)" style="box-shadow: inset 2px 0 0 #0f834d;height:40px;width:100%;background-color: #f2f2f2;border: none;" name="cielo_webservice[parcela]" id="parcela-cielo-webservice-credito">
...
<?php 
$bandeira_selecionada = WC()->session->get('bandeirac_cielo_loja_5');
if(!empty($bandeira_selecionada)){
?>
aplicar_bandeira_cielo_webservice('<?php echo $bandeira_selecionada;?>');
<?php } ?>
function autenticar_credito(par){
	console.log('credito valid');
	if(par.value!=''){
		//desativa o button
		jQuery('#place_order').attr("disabled","disabled");
		//default
		jQuery('#bpmpi_paymentmethod').val('credit');
		//clona
		console.log(jQuery(par).find(":selected").data('parcela'));
		jQuery('#bpmpi_cardnumber').val(jQuery('input[name="cielo_webservice[numero]"]').val().replace(/\s/g, ""));
		jQuery('#bpmpi_installments').val(jQuery(par).find(":selected").data('parcela'));
		jQuery('#bpmpi_cardexpirationmonth').val(jQuery('#validade-cielo-webservice-mes').val());
		jQuery('#bpmpi_cardexpirationyear').val(jQuery('#validade-cielo-webservice-ano').val());
		//auth 
