<?php
class RestClientExceptionCielo extends Exception {}

class RestClientCielo implements Iterator, ArrayAccess {
    ...
    public function offsetSet($key, $value){
        throw new RestClientExceptionCielo("Decoded response data is immutable.");
    }
    ...
    public function execute($url, $method='GET', $parameters=array(), $headers=array()){
        ...
        $curlopt = array(
			...
			CURLOPT_SSLVERSION => CIELO_WEBSERVICE_WOO_CURL_SSL,
		...
		$client->status = curl_getinfo($client->handle, CURLINFO_HTTP_CODE);
    ...
    public function get_response_format(){
        if(!$this->response)
            throw new RestClientExceptionCielo(
    ...
    public function decode_response(){
        if(empty($this->decoded_response)){
            ...
            if(!array_key_exists($format, $this->options['decoders']))
                throw new RestClientExceptionCielo("'${format}' is not a supported ".
