...
function lastudio_element_add_controls_group_to_element( $element ){
    ...
    if(lastudio_get_theme_support('lastudio-kit::motion-effects')){
        return;
    }
...
function lastudio_element_add_sticky_control_to_element( $element ){
    ...
    if(lastudio_get_theme_support('lastudio-kit::sticky-control')){
        return;
    }
...
if(!defined('ELEMENTOR_PRO_VERSION')) {
    ...
    add_shortcode( 'elementor-template', function( $attributes = [] ){
        ...
        if( (lastudio_get_theme_support('lastudio-kit::ajax-templates') || lastudio_get_theme_support('elementor::ajax-templates')) && !\Elementor\Plugin::instance()->editor->is_edit_mode() && !\Elementor\Plugin::instance()->preview->is_preview_mode() ){
            $template_content = '<div data-ajaxtmp="true" data-template-id="'.esc_attr($attributes['id']).'"><div class="lakit-hamburger-panel-loader lakit-tpl-panel-loader lakit-css-loader"></div></div>';
        }
        else{
...
add_filter('single_template', function( $template ){
    if(is_singular('elementor_library') && !defined('ELEMENTOR_PRO_VERSION')){
        if(lastudio_get_theme_support('lastudio-kit::template_library')){
            return $template;
        }
...
function lastudio_element_modify_content_width_for_section( $element, $args ){
    if ( version_compare( ELEMENTOR_VERSION, '3.3.0', '<' ) ) {
