...
if ( ! class_exists( 'LAHB_Nav_Walker' ) ) :
    ...
    class LAHB_Nav_Walker extends Walker_Nav_Menu{
        ...
        public function mega_start_el( &$output, $item, $depth=0, $args=array(),$current_object_id=0 ){
            ...
            if ($depth == 0 && in_array( 'menu-item-has-children', $classes, true )) {
                ...
                if ($item->menu_type == "wide") {
                    ...
                    if(!empty($item->popup_background)){
                        ...
                        $new_pp = [];
                        foreach ($item->popup_background as $ppk => $ppv){
                            if( 'background-image' == $ppk ){
                                $new_pp['image'] = $ppv['url'];
                            }
                            else{
                                $new_pp[ str_replace('background-', '', $ppk) ] = $ppv;
                            }
                        }
                        $item->popup_background = $new_pp;
                        ...
                        if(!empty($popup_background['image']) || !empty($popup_background['color'])){
                            $classes[] = 'mm--has-bgsub';
                        }
            ...
            if(!empty($item->icon)){
                $item_output .= '<i class="mm-icon la-menu-icon '.$item->icon.'"></i>';
            }
